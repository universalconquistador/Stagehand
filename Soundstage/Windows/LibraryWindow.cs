...
using Soundstage.Services;
using Soundstage.Utils;
using System;
...
{
    private const string LibraryWindowCommand = "/soundstage";
    ...
    private readonly LocalSoundstageService _localSoundstageService;
    ...
    public LibraryWindow(ILogger<LibraryWindow> logger, IDalamudPluginInterface dalamudPluginInterface, ICommandManager commandManager, IDataManager dataManager, IClientState clientState, ILocalDefinitionService localDefinitionService, ILiveSoundstageService liveSoundstageService, LocalSoundstageService localSoundstageService, WindowSystem windowSystem, SoundstageConfiguration configuration) : base($"Soundstage {dalamudPluginInterface.Manifest.AssemblyVersion}###SoundstageLibrary")
    {
        ...
        _localSoundstageService = localSoundstageService;
    ...
    public override void Draw()
    {
        using (ImRaii.PushStyle(ImGuiStyleVar.CellPadding, new Vector2(ImGui.GetStyle().CellPadding.X, 0.0f)))
        {
            using (ImRaii.Table("librarywindow", 2, ImGuiTableFlags.Resizable | ImGuiTableFlags.NoBordersInBodyUntilResize))
            {
                ...
                using (var listBox = ImRaii.ListBox("###soundstages", ImGui.GetContentRegionAvail() - new Vector2(0.0f, bottomBarHeight + ImGui.GetStyle().ItemSpacing.Y)))
                {
                    if (listBox.Success)
                    {
                        ...
                        using (ImRaii.PushStyle(ImGuiStyleVar.ItemSpacing, Vector2.Zero))
                        {
                            foreach (var localDefinition in _localDefinitionService.LocalDefinitions.OrderBy(pair => pair.Key, PathSorter.CurrentCultureIgnoreCase))
                            {
                                ...
                                {
                                    // The file's treenode
                                    bool isVisible = _liveSoundstageService.TryGetLiveSoundstage(LiveSoundstageHelpers.MakeLocalSoundstageKey(localDefinition.Key), out _);
                                    using (ImRaii.PushFont(UiBuilder.IconFont))
                ...
                using (var popup = ImRaii.Popup("CreateNewDefinition"))
                {
                    if (popup.Success)
                    {
                        if (ImGui.InputTextWithHint("Name", "Name", _newDefinitionFilenameBuffer, ImGuiInputTextFlags.EnterReturnsTrue))
                        {
                            ...
                            var newInfo = new SoundstageInfo()
                            {
                                Name = name,
                                AuthorName = "",
                                Description = "New Soundstage",
                                VersionString = "0.0.1",
                                IntendedTerritoryType = _clientState.TerritoryType,
                            };

                            var newDefinition = new SoundstageDefinition() { Info = newInfo };
                            ...
                            {
                                _logger.LogError(ex, "Failed to write new soundstage definition file {file}!", finalPath);
                            }
                        }
                    }
                }
                ...
                if (!string.IsNullOrEmpty(_selectedLocalDefinitionFilename) && _localDefinitionService.LocalDefinitions.TryGetValue(_selectedLocalDefinitionFilename, out var selectedMetadata))
                {
                    ...
                    string liveKey = LiveSoundstageHelpers.MakeLocalSoundstageKey(_selectedLocalDefinitionFilename);
                    bool isVisible = _liveSoundstageService.TryGetLiveSoundstage(liveKey, out _);

                    if (isVisible)
                    {
                        if (ImGui.Button("Hide"))
                        {
                            _liveSoundstageService.TryDestroyLiveSoundstage(liveKey);
                            _localSoundstageService.SetManualVisibility(_selectedLocalDefinitionFilename, false);
                        }
                    }
                    else
                    {
                        ...
                        {
                            _localSoundstageService.SetManualVisibility(_selectedLocalDefinitionFilename, true);
                            try
                            {
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            HelpMessage = "Show the Soundstage main window"
        });
