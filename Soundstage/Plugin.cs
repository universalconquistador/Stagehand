...
public sealed class Plugin : IDalamudPlugin
{
    ...
    public SoundstageConfiguration Configuration { get; init; }
    
    public readonly WindowSystem WindowSystem = new("Soundstage");
    ...
    public Plugin()
    {
        ...
        Configuration = PluginInterface.GetPluginConfig() as SoundstageConfiguration ?? new SoundstageConfiguration();
        if (string.IsNullOrEmpty(Configuration.DefinitionLibraryPath))
        {
            Configuration.DefinitionLibraryPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Soundstages");
            Configuration.Save();
        }
        ...
        hostBuilder.ConfigureServices(services =>
        {
            ...
            services.AddSingleton<ILiveSoundstageService, LiveSoundstageService>();

            services.AddHostedService<ConfigWindow>();
            services.AddHostedService<DebugWindow>();
            services.AddHostedService<LibraryWindow>();
            services.AddSingleton<LocalSoundstageService>();
            services.AddHostedService(c => c.GetRequiredService<LocalSoundstageService>());
        });
