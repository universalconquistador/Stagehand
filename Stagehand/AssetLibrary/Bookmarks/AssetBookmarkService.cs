...
public interface IAssetBookmarkService
{
    ...
    /// <summary>
    /// Attempts to find the bookmark item specified by a path generated by a call to
    /// <see cref="BookmarkItemExtensions.ToBookmarkPath(IBookmarkItem)"/>.
    /// </summary>
    /// <param name="bookmarkItemPath">The bookmark path.</param>
    /// <param name="item">The bookmark item if it was found.</param>
    /// <returns>Whether an item was found for the path.</returns>
    bool TryFindBookmarkItemFromPath(string bookmarkItemPath, [NotNullWhen(true)] out IBookmarkItem? item);
}
...
internal partial class AssetBookmarkService : IAssetBookmarkService, IDisposable
{
    ...
    public bool TryFindBookmarkItemFromPath(string bookmarkItemPath, [NotNullWhen(true)] out IBookmarkItem? item)
    {
        ReadOnlySpan<char> path = bookmarkItemPath;
        BookmarkItem? bookmarkItem = null;
        while (path.Length > 0)
        {
            var periodIndex = path.LastIndexOf('.');
            if (Guid.TryParse(path.Slice(periodIndex + 1), out var guid))
            {
                if (bookmarkItem != null)
                {
                    if (bookmarkItem is FolderBookmarkItem folderBookmarkItem)
                    {
                        bookmarkItem = folderBookmarkItem.ChildItems.FirstOrDefault(item => item.Guid == guid);
                    }
                    else
                    {
                        // One of the parents isn't actually a folder!
                        item = null;
                        return false;
                    }
                }
                ...
                {
                    bookmarkItem = _rootItemsSorted.FirstOrDefault(item => item.Guid == guid);
                }
            }

            if (periodIndex >= 0)
            {
                path = path.Slice(0, periodIndex);
            }
            else
            {
